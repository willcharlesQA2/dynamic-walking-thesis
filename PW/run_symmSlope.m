...
% masses = 0.75:0.001:1;
% radii = 0.15:0.001:0.43;
slopes = [0.01:0.02:10]*-pi/180;  % change back to 13
slopes = [0.01:0.02:6.5]*-pi/180;  % point foot
...
%                         h(:,2) = plot(r_B,Dist(end-15:2:end)./tf(end-14:2:end),'r.'); % prosthetic
    ...
    %% NEED TO FIGURE OUT HOW TO SAVE STABILITY POINTS FOR LATER USE
    % for n = 1:size(stability,3)
    %     [row(:,n),col(:,n)]=find(stability(:,:,n) == 1 );
    % end
    %
    %
    % %[ii,jj,kk] = find(stability == 1);
    % stable_points=[th1dot_stability(ii),th2dot_stability(jj),th1_stability(kk)]; %Stable points can be plotted later
    % first1    = th1dot_stability(I(1));
    % last1     = th1dot_stability(I(end));
    % first2    = th2dot_stability(I(1));
    % last2     = th2dot_stability(I(end));
...
% save('ROCslope0p25V2.mat')
save('symmetric_P_SmallV2.mat')
