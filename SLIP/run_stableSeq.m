...
ME_total = linspace(800,900,111);
alpha_total = linspace(14,25,111)*-pi/180;
k_total = linspace(17,35,181)*1000;
...
%% Plot step diagram
figure
plot(x_axis,GCrdot,'-*','MarkerEdgeColor','k','MarkerSize',3)
...
%% Y phase-plane
[X1,Y1,X1dot,Y1dot] = locations(t1,u1,Param,step);
[X2,Y2,X2dot,Y2dot] = locations(t2,u2,Param,step);

figure
hold on
LW = 2;
plot(Y1,Y1dot,'LineWidth',LW)
plot(Y2,Y2dot,'r','LineWidth',LW)
% title('Phase-plane diagram of SLIP model')
xlabel('COM Y position (m)','FontSize',FS)
ylabel('COM Y velocity (m/s)','FontSize',FS)
set(gca,'FontSize',13)
box on