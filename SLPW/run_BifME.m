...
% Potential Energy of standing
standingPE = (Param.m3*L + Param.mA*Param.LrA + Param.mB*Param.LrB)*9.81;
...
stiffnesses = linspace(12e3,25e3,sizeB); % 5 -> 25
% stiffnesses = [11.4,12,12.6]*1e3;

Mus = linspace(3.45,3.85,sizeB);

Rockers = linspace(0.23,0.35,sizeB);
...
%                 [ step, speed, GRFs, Param] = Walk([uStable,stiffnesses(bb),slope,damping,ME]);
        ...
        if step == maxsteps
            ...
            Allth1(:,bifurcation) = results.th1End;
            Allth1dot(:,bifurcation) = results.th1dot;
            Allth2dot(:,bifurcation) = results.th2dot;
            Allr1(:,bifurcation) = results.rEnd;
            Allr1dot(:,bifurcation) = results.rdotEnd;
            ...
            Allu1End{:,bifurcation} = results.u1End;
            Allu2Start{:,bifurcation} = results.u2Start;
            ...
            for imp = 1:15
                ...
                rdotDiff(imp,bifurcation) = AllrdotStart(imp+1,bifurcation)-AllrdotEnd(imp,bifurcation);
