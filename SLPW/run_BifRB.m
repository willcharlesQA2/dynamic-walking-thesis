...
%         Param.mB = Param.m3/Mus(bb);
        ...
        if step ~= maxsteps
            [ uStableNew, step, speed, GRFs, Param, results] = findStableRAND(Param,100); % was 100
...
%                 [ step, speed, GRFs, Param] = Walk([uStable,stiffnesses(bb),slope,damping,ME]);
        ...
        if step == maxsteps
            ...
            if size_pks(:,bifurcation) == 2
                GRFPeak1(:,bifurcation) = Allpks(1,:);
                GRFPeak2(:,bifurcation) = Allpks(2,:);
            else
                GRFPeak1(:,bifurcation) = NaN;
                GRFPeak2(:,bifurcation) = NaN;
            end
...
Sym = 0.3;
save('resultsBifRBSwoma3V3V3.mat')
