...
% Foot parameters
%% Foot geometry
s1 = 0;%-0.4712;      % hindfoot length
s2 = 0;          %
s3 = 0;
s4 = 0;%0.4712;       % forefoot length
%
% %% Point foot
% s1 = 0;
% s4 = 0;
...
% Used to be stiffnesses = linspace(10e3,35e3,sizeB);
stiffnesses = linspace(10e3,35e3,sizeB); % 5 -> 25
% stiffnesses = [11.4,12,12.6]*1e3;

%minimum ME (I think) is 667.08. Based on th1 = 0, th2 = 0 and KE = 0
% Used to be MEs = linspace(685,735,sizeB);
MEs = linspace(690,725,sizeA);

Mus = linspace(1.4,4,sizeB);
Betas = linspace(0.5,0.7,sizeB);
...
BOApks = BOA;
% 
% % Don't do this. It doesn't work for cell objects
% Allu2Start = BOA;
% Allu1End = BOA;
...
AllAlpha = nan(sizeA,sizeB,16);
AllrdotStart    = AllAlpha;
AllrdotEnd      = AllAlpha;
Allth1dot       = AllAlpha;
Allth1dotStart  = AllAlpha;
AllSpeeds       = AllAlpha;
...
%         x_axis = Betas(bb);
        ...
        if step ~= maxsteps
            [ uStableNew, step, speed, GRFs, Param, results] = findStableRAND(Param,50);
...
%             AllM_total(:,bifurcation) = Param.M_total;
            AllAlpha(aa,bb,:)       = results.touchDownAngle;
            AllrdotStart(aa,bb,:)   = results.rdotStart;
            AllrdotEnd(aa,bb,:)     = results.rdotEnd;
            Allth1dot(aa,bb,:)   = results.th1dot;
            Allth1dotStart(aa,bb,:)   = results.th1dotStart;
            AllSpeeds(aa,bb,:) = speed;
            Allu1End{aa,bb} = results.u1End;
            Allu2Start{aa,bb} = results.u2Start;
...
%             GRFPeak2(:,bifurcation) = Allpks(2,:);
    ...
    fprintf('\naa = %g. Estimate time left is %g hours.\n',aa,toc*(sizeA/aa-1)/3600)
end
